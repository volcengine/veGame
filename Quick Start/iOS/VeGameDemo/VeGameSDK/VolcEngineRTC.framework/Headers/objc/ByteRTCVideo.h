 ...
 *          通过此接口获取授权消息后，参考 [在线授权说明](http://ailab-cv-sdk.bytedance.com/docs/2036/99798/)，自行实现获取在线许可证的业务逻辑。获取许可证后，你必须调用 checkVideoEffectLicense{@link #ByteRTCVideo#checkVideoEffectLicense} 确认许可证有效。然后，你才可以使用 CV 功能。  <br>
 */
 ...
 * @notes 要设置音频路由，详见 setAudioRoute:{@link #ByteRTCVideo#setAudioRoute:}。
 */
 ...
 * @notes <br>
 *     + 本方法只会停止本地屏幕采集，并不会影响屏幕流的发布状态，再次调用 startScreenCapture:bundleId:{@link #ByteRTCVideo#startScreenCapture:bundleId:} 开始屏幕音视频内部采集。
 ...
 *       + 可以调用 registerVideoFrameObserver:{@link #ByteRTCVideo#registerVideoFrameObserver:} 注册视频回调对象，通过回调 onLocalScreenFrame:{@link #ByteRTCVideoFrameObserver#onLocalScreenFrame:} 获取采集成功的本地视频帧。 <br>
 */
 ...
 *      + 如果设定为内部采集，你必须再调用 startScreenCapture:bundleId:{@link #ByteRTCVideo#startScreenCapture:bundleId:} 开始采集。
 *      + 如果设定为自定义采集，你必须再调用 pushScreenAudioFrame:{@link #ByteRTCVideo#pushScreenAudioFrame:} 将自定义采集到的屏幕音频帧推送到 RTC SDK。<br>
 *      + 无论是内部采集还是自定义采集，你都必须调用 publishScreen:{@link #ByteRTCRoom#publishScreen:} 将采集到的屏幕音频推送到远端。
 */
