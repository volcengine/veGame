...
public:
   ...
   virtual void onUserAudioSendModeChange(const char* uid, int range_audio_mode) {
     ...
     * @param [in] deviceType 设备类型，详见 MediaDeviceType{@link #MediaDeviceType}
     ...
     * @param [in] device_id 设备 ID
     * @param [in] deviceType 详见 MediaDeviceType{@link #MediaDeviceType}
     ...
     *        消息发送结果  <br>
     *        详见 UserMessageSendResult{@link #UserMessageSendResult}。
     * @param [in] msg 应用服务器收到 HTTP 请求后，在 ACK 中返回的信息。
     * @notes 当调用 sendServerMessage{@link #IRtcEngine#sendServerMessage} 或 sendServerBinaryMessage{@link #IRtcEngine#sendServerBinaryMessage} 接口发送消息后，会收到此回调。
     */
