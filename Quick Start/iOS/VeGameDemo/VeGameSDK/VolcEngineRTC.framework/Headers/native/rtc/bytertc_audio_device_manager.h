...
public:
     ...
     * @type api
     * @region 音频设备管理
     * @brief 启动音频播放设备检测。测试启动后，循环播放指定的音频文件，同时将通过 onAudioPlaybackDeviceTestVolume{@link #IRTCVideoEventHandler#onAudioPlaybackDeviceTestVolume} 回调播放时的音量信息。
     * @param [in] test_audio_file_path 指定播放设备检测的音频文件网络地址。支持的格式包括 mp3，aac，m4a，3gp 和 wav。
     * @param [in] indication_interval 设置 onAudioPlaybackDeviceTestVolume{@link #IRTCVideoEventHandler#onAudioPlaybackDeviceTestVolume} 音量回调的时间间隔，推荐设置为 200 毫秒或以上。单位为毫秒。最小值为 10 毫秒。 
     * @return  方法调用结果  <br>
     *        + 0：方法调用成功  <br>
     *        + < 0：方法调用失败  <br>
     * @notes  <br>
