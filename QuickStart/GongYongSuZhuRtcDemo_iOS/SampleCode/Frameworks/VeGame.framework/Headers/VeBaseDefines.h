...
typedef NS_ENUM(NSUInteger, VeBaseMediaDeviceState) {
    ...
    /** 设备已插入 */
    VeBaseMediaDeviceStateAdded = 10,
    /** 设备被移除 */
    VeBaseMediaDeviceStateRemoved = 11,
    /** 系统通话，锁屏或第三方应用打断了音视频通话。将在通话结束或第三方应用结束占用后自动恢复 */
    VeBaseMediaDeviceStateInterruptionBegan = 12,
    /** 音视频通话已从系统电话或第三方应用打断中恢复 */
    VeBaseMediaDeviceStateInterruptionEnded = 13
};

typedef NS_ENUM(NSInteger, VeBaseMediaDeviceError) {
    /** 媒体设备正常 */
    VeBaseMediaDeviceErrorOK = 0,
    /** 没有权限启动媒体设备 */
    VeBaseMediaDeviceErrorDeviceNoPermission = 1,
    /** 媒体设备已经在使用中 */
    VeBaseMediaDeviceErrorDeviceBusy = 2,
    /** 媒体设备错误 */
    VeBaseMediaDeviceErrorDeviceFailure = 3,
    /** 未找到指定的媒体设备 */
    VeBaseMediaDeviceErrorDeviceNotFound = 4,
    /** 媒体设备被移除 */
    VeBaseMediaDeviceErrorDeviceDisconnected = 5,
    /** 无采集数据。当媒体设备的预期行为是正常采集，但没有收到采集数据时，将收到该错误 */
    VeBaseMediaDeviceErrorDeviceNoCallback = 6,
    /** 设备采样率不支持 */
    VeBaseMediaDeviceErrorUNSupportFormat = 7,
    /** iOS 屏幕采集没有 group Id 参数 */
    VeBaseMediaDeviceErrorNotFindGroupId = 8,
    /** 视频采集中断：因用户使用系统相机，应用切换到后台运行，导致采集中断 */
    VeBaseMediaDeviceErrorNotAvailableInBackground = 9,
    /** 视频采集中断：可能由于其他应用占用系统相机，导致视频设备暂时不可用，从而造成采集中断 */
    VeBaseMediaDeviceErrorVideoInUseByAnotherClient = 10,
    /** 视频采集中断：当前应用处于侧拉、分屏或者画中画模式时，导致采集中断 */
    VeBaseMediaDeviceErrorNotAvailableWithMultipleForegroundApps = 11,
    /** 视频采集中断：由于系统性能不足导致中断，比如设备过热 */
    VeBaseMediaDeviceErrorNotAvailableDueToSystemPressure = 12
};
