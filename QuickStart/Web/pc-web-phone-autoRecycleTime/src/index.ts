...
const startSuccessCallback = (gameController) => {
  ...
  // 初始化自动回收时长
  const autoRecycleTimeInput = document.getElementById(
    "auto-recycle-time"
  ) as HTMLInputElement;
  autoRecycleTimeInput.value = gameController.sdkInstance
    .getAutoRecycleTime()
    .toString();

  // 绑定提交修改按钮事件
  const submitButton = document.getElementById("submit-recycle-time");
  if (submitButton) {
    submitButton.addEventListener("click", async () => {
      const autoRecycleTime = autoRecycleTimeInput?.value?.trim();
      if (autoRecycleTime !== undefined && autoRecycleTime !== "") {
        ...
        await gameController.sdkInstance.setAutoRecycleTime(
          Number(autoRecycleTime)
        );
        console.log(`自动回收时长已设置为: ${autoRecycleTime}秒`);
      } else {
        console.log("自动回收时长输入框不存在有效值");
      }
    });
  }

  // 绑定获取当前值按钮事件
  const getButton = document.getElementById("get-recycle-time");
  if (getButton) {
    getButton.addEventListener("click", async () => {
      const { duration } =
        await gameController.sdkInstance.getAutoRecycleTime();
      if (autoRecycleTimeInput) {
        autoRecycleTimeInput.value = duration.toString();
      }
      console.log(`当前自动回收时长为: ${duration}秒`);
    });
  }
};
