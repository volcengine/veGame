# 云游戏客户端 Web SDK Sample - PC 玩手游 - 坐标旋转示例

## 功能说明

这个示例展示了如何在 PC 端玩手游场景下坐标旋转。当使用 CSS transform 的 rotate/rotate3d 进行 UI 旋转操作时，需要配合调用该方法进行坐标旋转，保证点击/触控坐标计算准确。
...
```bash
npm run dev
cd QuickStart/Web/pc-web-phone-rotation
```
